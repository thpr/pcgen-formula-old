...
public class AggressiveSolverManagerTest extends AbstractFormulaTestCase
{
	...
	public void testIllegalAddModifier()
	{
		...
		try
		{
			manager.addModifier(hp, null, source);
			fail();
		}
		catch (IllegalArgumentException e)
		{
		...
		try
		{
			manager.addModifier(hp, modifier, null);
			fail();
		}
		catch (IllegalArgumentException e)
		{
		...
		try
		{
			manager.addModifier(brains, modifier, source);
	...
	public void testComplex()
	{
		...
		manager.addModifier(arms, two, source);
		...
		manager.addModifier(legs, four, source);
		assertEquals(2, store.get(arms));
		assertEquals(4, store.get(legs));
		assertEquals(6, store.get(limbs));

		manager.removeModifier(arms, two, source);
		assertEquals(0, store.get(arms));
		assertEquals(4, store.get(legs));
	...
	public void testChained()
	{
		...
		assertEquals(0, store.get(arms));
		manager.addModifier(arms, handsMod, source);
		assertEquals(0, store.get(arms));

		AbstractModifier<Number> ten = AbstractModifier.setNumber(10, 5);
		manager.addModifier(fingers, ten, source);
		assertEquals(2, store.get(arms));
		assertEquals(2, store.get(limbs));
		...
		manager.removeModifier(arms, handsMod, source);
		assertEquals(0, store.get(arms));
		assertEquals(2, store.get(legs));
		assertEquals(2, store.get(limbs));
	...
	public void testIllegalRemoveModifier()
	{
		...
		try
		{
			manager.removeModifier(hp, null, source);
			fail();
		}
		catch (IllegalArgumentException e)
		{
		...
		try
		{
			manager.removeModifier(hp, modifier, null);
			fail();
		}
		catch (IllegalArgumentException e)
		{
		...
		try
		{
			manager.removeModifier(brains, modifier, source);
			fail("Didn't own that VarID!");
		}
		catch (IllegalArgumentException e)
		{
	...
	public void testCircular()
	{
		...
		try
		{
			manager.addModifier(fingers, fingersMod, source);
			fail("How?");
		}
